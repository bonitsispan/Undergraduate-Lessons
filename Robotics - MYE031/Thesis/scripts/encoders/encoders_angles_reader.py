...
output_file = open("encoders_angles.txt", "w")
output_file.write("Theta1 (degrees), Theta2 (degrees)\n")  # Add header to the file
...
try:
    while True:
        ...
        # Read the angular position of encoder 1 (theta1)
        theta1 = task0.read()

        # Read the angular position of encoder 2 (theta2)
        theta2 = task1.read()

        print(f"Theta1: {theta1:.2f} degrees, Theta2: {theta2:.2f} degrees")

        # Write the positions to the file
        output_file.write(f"{theta1:.2f}, {theta2:.2f}\n")
