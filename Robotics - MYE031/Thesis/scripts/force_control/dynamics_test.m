...
while true
    ...
    %T_static_friction = 0 * sign(dq(:,k-1));
...
% Plot
figure;
plot(t(1:k-1), xe(1:k-1), 'b-', 'LineWidth', 2, 'DisplayName', 'Position - Xe');
%plot(t(1:k-1), dq(1,1:k-1), 'b-', 'LineWidth', 2, 'DisplayName', 'Velocity - Theta1');

hold on;
plot(t(1:k-1), ye(1:k-1), 'r-', 'LineWidth', 2, 'DisplayName', 'Position - Ye');
%plot(t(1:k-1), dq(4,1:k-1), 'r-', 'LineWidth', 2, 'DisplayName', 'Velocity - Theta2');
...
%ylabel('Velocity (rad/sec)', 'FontSize', 14, 'FontWeight', 'bold');
%ylim([-4.5 4.5]);

%legend('x_e', 'y_e');
legend('Location', 'northeast', 'FontSize', 12);
%legend('Velocity - Theta1', 'Velocity - Theta2');
